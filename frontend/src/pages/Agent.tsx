...
const Agent = () => {
  ...
  const [selectedWebsite1, setSelectedWebsite1] = useState<any>(null)
  ...
  const sendAudioToServer = async (audioBlob: Blob) => {
    try {
      ...
      formData.append("vulnerabilities", JSON.stringify(selectedWebsite?.vulnerabilities));

      const response = await fetch("http://127.0.0.1:5001/api/voice-agent", {
        method: "POST",
  ...
  const vulnerabilityCounts = {
    critical: selectedWebsite?.vulnerabilities?.filter((v) => v.severity === "critical").length || 0,
    high: selectedWebsite?.vulnerabilities?.filter((v) => v.severity === "high").length || 0,
    medium: selectedWebsite?.vulnerabilities?.filter((v) => v.severity === "medium").length || 0,
    low: selectedWebsite?.vulnerabilities?.filter((v) => v.severity === "low").length || 0,
  }

  const totalVulnerabilities = selectedWebsite?.vulnerabilities?.length || 0

  // Calculate security score (simple algorithm - can be refined)
  const calculateSecurityScore = () => {
    if (!totalVulnerabilities) return 100

    const weights = { critical: 25, high: 15, medium: 7, low: 3 }
    const weightedSum = Object.entries(vulnerabilityCounts).reduce(
      (sum, [severity, count]) => sum + weights[severity] * count,
      0,
    )
      ...
      <div className="flex-1 ml-64">
        ...
        <div className="p-6">
          ...
          <div className="max-w-7xl mx-auto">
                  ...
                  {scans.map((scan) => {
                    ...
                    return (
                      <motion.div
                        ...
                        onClick={() => {setSelectedWebsite(scan); setSelectedWebsite1(scan)}}
                      >
            ...
            <AnimatePresence>
              {selectedWebsite1 && (
                <Dialog open={!!selectedWebsite1} onOpenChange={() => setSelectedWebsite1(null)}>
                  <DialogContent className="sm:max-w-[425px] bg-white border border-gray-200 shadow-xl rounded-xl">
                    <DialogHeader>
                      ...
                      <DialogDescription className="text-gray-600">
                        Begin an AI-guided analysis of security vulnerabilities for {selectedWebsite1?.website_link}
                      </DialogDescription>
                    </DialogHeader>
            ...
            <div className="w-full max-w-7xl mx-auto">
              {showAICall && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  {/* Left Side - Vulnerability Summary */}
                  <div className="bg-white rounded-2xl p-8 shadow-xl border border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      {Object.entries(vulnerabilityCounts).map(([severity, count]) => (
                        <Card key={severity} className="border-0 shadow-md overflow-hidden">
                          <CardContent className="p-0">
                            <div className={`flex items-center justify-between p-4 ${getSeverityColor(severity)}`}>
                    ...
                    <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                      <h3 className="text-lg font-semibold text-gray-800 mb-2">Detected Vulnerabilities</h3>

                      {selectedWebsite?.vulnerabilities?.map((vulnerability, index) => (
                        <Card
                          key={index}
                          className={`border-l-4 ${vulnerability.severity === 'critical' ? 'border-l-red-500' :
                        ...
                        >
                          <CardHeader className="p-4 pb-2">
                            <div className="flex justify-between items-start">
                              ...
                              <Badge className={`${getSeverityColor(vulnerability.severity)} capitalize`}>
                          ...
                          {expandedVulnerability === index && (
                            <CardContent className="p-4 pt-2 bg-gray-50 rounded-b-lg animate-fadeIn">
                              <div className="text-sm text-gray-700 mb-3">
                                {vulnerability.description}
                              </div>

                              {vulnerability.leaked_value?.length > 0 && (
                                <div className="mb-3">
                                  <h4 className="text-xs font-semibold text-gray-600 mb-1">Leaked Values:</h4>
                                  <div className="bg-gray-100 p-2 rounded text-xs font-mono text-gray-800 max-h-20 overflow-y-auto">
                                    {vulnerability.leaked_value.map((value, i) => (
                                      <div key={i} className="mb-1 last:mb-0 break-all">{value}</div>
                                    ))}
                                  </div>
                                </div>
                              )}
                              ...
                              <div>
                                <h4 className="text-xs font-semibold text-gray-600 mb-1">Recommendation:</h4>
                                <p className="text-xs text-gray-700">{vulnerability.recommendation}</p>
                              </div>
                            </CardContent>
                          )}
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-gray-700">Scanned Website:</span>
                        </div>
                        <a
                          href={selectedWebsite?.website_link}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sm text-indigo-600 hover:text-indigo-800 flex items-center gap-1"
                        >
                          {selectedWebsite?.website_link} <ExternalLink className="h-3 w-3" />
                        </a>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-white to-indigo-50 rounded-2xl p-8 shadow-xl border border-indigo-100">
                    <div className="flex flex-col items-center">
                      {/* AI Avatar */}
                      <div className="w-32 h-32 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mb-8 shadow-lg p-2 border-4 border-white">
                      ...
                      <div className="space-y-4 w-full">
                        {!isCallStarted ? (
                          <Button
                            ...
                            className="w-full py-8 rounded-xl text-xl font-medium bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white shadow-lg transform hover:scale-105 transition-all"
                          >
                            Start Analysis
                          </Button>
                        ) : (
                          <>
                            <Button
                              ...
                              className={`w-full py-8 rounded-xl text-xl font-medium ${isRecording
                            ...
                            <Button
                              ...
                              className="w-full py-8 rounded-xl text-xl font-medium bg-gradient-to-r from-indigo-500 to-violet-500 hover:from-indigo-600 hover:to-violet-600 text-white shadow-lg transform hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              {isSending ? (
                                <>
                                  <Loader2 className="mr-2 h-5 w-5 animate-spin" />
                            ...
                            <Button
                              onClick={endCall}
                              className="w-full py-6 rounded-xl text-lg font-medium bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 shadow-md transform hover:scale-105 transition-all"
                            >
                              End Analysis
                            </Button>
                          </>
                      ...
                      <div className="mt-8 w-full">
                        <Card className="bg-white/70 border border-indigo-100">
                          <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-gray-700">How It Works</CardTitle>
                          </CardHeader>
                          <CardContent className="pt-0">
                            <p className="text-xs text-gray-600">
                              Our AI Security Analyst uses advanced algorithms to detect vulnerabilities in your website.
                              Speak naturally to ask questions about the detected issues and get personalized recommendations.
                            </p>
                          </CardContent>
                        </Card>
                      </div>
